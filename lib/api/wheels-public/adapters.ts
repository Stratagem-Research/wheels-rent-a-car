...
export function fromBookingDraft(
  ...
  return {
    ...
    pickup_address: draft.pickup.locationId ?? draft.pickup.address,
    drop_off_address: draft.return.locationId ?? draft.return.address,
...
/**
 * Persisted mapping from a synthesized `WRC-...` ref back to the backend's
 * numeric ids and the email used at booking — needed because the public API
 * has no lookup endpoint. The manage-booking and confirmation pages read
 * this map (browser-side) to render previously-created bookings.
 */
export interface BookingRefMapEntry {
  ref: BookingRef;
  bookingId: number;
  numericId: number;
  email: string;
...
export const BOOKING_REF_MAP_STORAGE_KEY = "wheels.booking.refMap";

export function readRefMap(storage: Storage): Record<BookingRef, BookingRefMapEntry> {
  try {
    const raw = storage.getItem(BOOKING_REF_MAP_STORAGE_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw) as Record<string, BookingRefMapEntry>;
...
export function writeRefMap(
  storage: Storage,
  entry: BookingRefMapEntry,
): Record<BookingRef, BookingRefMapEntry> {
  const current = readRefMap(storage);
  current[entry.ref] = entry;
  try {
    storage.setItem(BOOKING_REF_MAP_STORAGE_KEY, JSON.stringify(current));
  } catch {
    // Quota or privacy mode — swallow; the ref map is a UX nicety, not a
    // source of truth.
  }
  return current;
}
